...
void OptionsGamestate::clickedAbout() {
    qDebug("about");
    QString text = tr("Arkanoid Game by 1363");
    QMessageBox::about(mainWindow, tr("About Arkanoid"), text);
}

void OptionsGamestate::clickedHelp() {
    qDebug("help");
    QDesktopServices::openUrl(QUrl(tr("http://vk.com/fkti1363")));
}
...
void OptionsGamestate::start() {
    ...
    QLabel *titleLabel = new QLabel(tr("Arkanoid"));
...
bool PlayingGamestate::userQuit() {
    ...
    paused = true;
    if( QMessageBox::question(this->mainWindow, "Arkanoid", "Are you sure you wish to quit?", QMessageBox::Ok, QMessageBox::Cancel) == QMessageBox::Ok ) {
        qDebug("quit");
        OptionsGamestate *gamestate = new OptionsGamestate(mainWindow);
        mainWindow->setGamestate(gamestate);
